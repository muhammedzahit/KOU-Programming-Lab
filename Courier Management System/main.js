...
const INITIAL_LOGIN_USERNAME = "admin@admin.com"
const INITIAL_LOGIN_PASSWORD = "admin"
const YOUR_GOOGLE_MAPS_API_KEY = "?????"
window.onload = () => {
    document.getElementById("key").src = "https://maps.googleapis.com/maps/api/js?key=" + YOUR_GOOGLE_MAPS_API_KEY + "&callback=initMap&v=weekly"
}
...
async function initDatabase() {
    ...
    let res = await db_login.find({})
    if (res.length == 0){
        await db_login.create({
            username : INITIAL_LOGIN_USERNAME,
...
function initMap() {
    ...
    async function initLocations(type) {
        ...
        if (type == "courier_locations") {
            ...
            if(result_.length == 0){
                await db_courier_locations.create({
                    lat : "40.76",
                    lng : "29.91",
                    address : "Seka Park",
                    email : find_dict[email]
                })
                await db.db_cargo_address.create({
                    lat : "40.76",
                    lng : "29.91",
                    address: "Seka Park"
                })
